...
type PermisJoin = Pick<Tables["permis"]["Row"], "code"> | null;
type InspecteurJoin = Pick<Tables["inspecteurs"]["Row"], "nom" | "prenom"> | null;
...
export function useEleves() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((e) => {
        const permis = e.permis as PermisJoin;
        const inspecteur = e.inspecteurs as InspecteurJoin;

        return {
          ...e,
          dossier_code: e.id.slice(0, 8).toUpperCase(),
          type_permis: permis?.code || "B",
          inspecteur: inspecteur ? `${inspecteur.prenom} ${inspecteur.nom}` : null,
...
export function useStudent(id: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const permis = data.permis as PermisJoin;
